...
Attribute VB_Exposed = False
  ...
  Event SkinChanged() '当完成加载皮肤时被激发  Raise when a skin is loaded
...
If Not bMax Then
        ...
        taskheight = RC.Bottom - RC.Top '任务栏高度 Taskbar Height
        i = GetWindowRect(GetDesktopWindow, RC)
        Dim maxwidth As Long
        Dim maxheight As Long
        maxwidth = RC.Right - RC.Left '获取屏幕宽度 Screen Width
...
'根据皮肤文件重新调整控件大小和位置  Resize and reposition all the controls according to the skin file
On Error Resume Next
Dim nFolderPath As String, nKeyVaule As String
    ' 判断路径是否合法 To see whether or not the path is valid
...
Public Function LoadSkin(SkinPath As String, Form As Form) ' 从路径加载自定义皮肤  Load a skin from a specific skin file
On Error GoTo err
Dim nFolderPath As String, nKeyVaule As String
    ' 判断路径是否合法 To see whether or not the path is valid
...
Public Function LimitSize(ByVal nWidth As Long, ByVal nHeight As Long) ' 限制窗口大小 Limit the size of the form
On Error Resume Next
    MinSizeX = nWidth
    MinSizeY = nHeight
    OldWindowProc = GetWindowLong(UserControl.ContainerHwnd, GWL_WNDPROC)
...
On Error Resume Next
'加载Vistor窗体   Draw Vistor Form
    With TheForm
        .AutoRedraw = True
        .ScaleMode = 3
        .Cls   '清除上次绘图痕迹    Clean The Form
        ...
        '.Refresh
    End With
    '绘制圆角窗体  Draw Rounded-rectangular Form
...
On Error Resume Next
   ...
   DeleteObject outRgn
   '重定义控件大小及位置   Resize and reposition all the controls
   With UserControl
      .Cls
